...
namespace ODataValidator.Rule
{
    ...
    public class ServiceImpl_SystemQueryOptionFilter_ToLower : ServiceImplExtensionRule
    {
        /// <summary>
        ...
        public override string Name
        {
            get
            {
                return "ServiceImpl_SystemQueryOptionFilter_ToLower";
            }
        }
        ...
        public override string Description
        {
            get
            {
                return this.CategoryInfo.CategoryFullName + ",$filter(tolower)";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            if (null != resp && HttpStatusCode.OK == resp.StatusCode)
            {
                ...
                var propVal = entity[propName].ToString().ToLower();
                url = string.Format("{0}?$filter=tolower({1}) eq '{2}'", url, propName, propVal);
                ...
                if (null != resp && HttpStatusCode.OK == resp.StatusCode)
                {
                    ...
                    {
                        passed = et[propName].ToString().ToLower() == propVal;
                    }
                }
                else
                {
