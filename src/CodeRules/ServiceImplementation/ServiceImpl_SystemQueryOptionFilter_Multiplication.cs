...
namespace ODataValidator.Rule
{
    ...
    public class ServiceImpl_SystemQueryOptionFilter_Multiplication : ServiceImplExtensionRule
    {
        /// <summary>
        ...
        public override string Name
        {
            get
            {
                return "ServiceImpl_SystemQueryOptionFilter_Multiplication";
            }
        }
        ...
        public override string Description
        {
            get
            {
                return this.CategoryInfo.CategoryFullName + ",$filter(Multiplication)";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            // Set the multiplier as 2.0. It is only a testing number, so it can be any numbers.
            double multiplier = 2.0;

            // Set the threshold as 10.0. It is only a testing number, so it can be any numbers.
            double threshold = 10.0;
            string url = string.Format("{0}/{1}?$filter={2} mul {3} gt {4}", svcStatus.RootURL.TrimEnd('/'), entitySetUrl, propName, multiplier, threshold);
