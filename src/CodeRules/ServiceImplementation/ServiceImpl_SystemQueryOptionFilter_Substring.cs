...
namespace ODataValidator.Rule
{
    ...
    public class ServiceImpl_SystemQueryOptionFilter_Substring : ServiceImplExtensionRule
    {
        /// <summary>
        ...
        public override string Name
        {
            get
            {
                return "ServiceImpl_SystemQueryOptionFilter_Substring";
            }
        }
        ...
        public override string Description
        {
            get
            {
                return this.CategoryInfo.CategoryFullName + ",$filter(substring)";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            if (null != resp && HttpStatusCode.OK == resp.StatusCode)
            {
                ...
                int startIndex = propVal.Length / 2;
                string subStr = propVal.Substring(startIndex);
                url = string.Format("{0}?$filter=substring({1}, {2}) eq '{3}'", url, propName, startIndex, subStr);
                ...
                if (null != resp && HttpStatusCode.OK == resp.StatusCode)
                {
                    ...
                    {
                        passed = et[propName].ToString().Substring(startIndex) == subStr;
                    }
                }
                else
                {
