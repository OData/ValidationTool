    ...
    public class ServiceImpl_EditMediaEntityStream : ServiceImplExtensionRule
    {
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            foreach (var en in entityTypeElements)
            {
                ...
                if (MetadataHelper.IsMediaEntity(serviceStatus.MetadataDocument, en.EntityTypeShortName, context) && !string.IsNullOrEmpty(eTypeElement.EntitySetName))
                {
                    eTypeElement = en;
                    break;
                }
            ...
            {
                detail1.ErrorMessage = "Cannot find the appropriate media entity type to verify this rule.";
                info = new ExtensionRuleViolationInfo(new Uri(serviceStatus.RootURL), serviceStatus.ServiceDocument, detail1);
            ...
            if (HttpStatusCode.Created == resp.StatusCode || HttpStatusCode.NoContent == resp.StatusCode)
            {
                ...
                {
                    passed = false;
                    detail3.ErrorMessage = string.Format("Fail to find the image read and edit URL.", resp.StatusCode);
                }                    
                ...
                if (resp.StatusCode.HasValue && HttpStatusCode.OK == resp.StatusCode)
                {
                    ...
                    detail3 = new ExtensionRuleResultDetail(this.Name, url, HttpMethod.Put, string.Empty, resp, string.Empty, "Successfully updated the stream of the image.");
                    ...
                    {
                        passed = false;
                        detail3.ErrorMessage = string.Format("HTTP PUT to update the image failed with the error {0}.", resp.StatusCode);
                    }
                ...
                {
                    passed = false;
                    detail2.ErrorMessage = "Get entity image failed from above URI.";
                }
