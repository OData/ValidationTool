...
namespace ODataValidator.Rule
{
    ...
    public class ServiceImpl_SystemQueryOptionFilter_ToUpper : ServiceImplExtensionRule
    {
        /// <summary>
        ...
        public override string Name
        {
            get
            {
                return "ServiceImpl_SystemQueryOptionFilter_ToUpper";
            }
        }
        ...
        public override string Description
        {
            get
            {
                return this.CategoryInfo.CategoryFullName + ",$filter(toupper)";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            if (null != resp && HttpStatusCode.OK == resp.StatusCode)
            {
                ...
                var propVal = entity[propName].ToString().ToUpper();
                url = string.Format("{0}?$filter=toupper({1}) eq '{2}'", url, propName, propVal);
                ...
                if (null != resp && HttpStatusCode.OK == resp.StatusCode)
                {
                    ...
                    {
                        passed = et[propName].ToString().ToUpper() == propVal;
                    }
                }
                else
                {
