...
namespace ODataValidator.Rule
{
    ...
    public class MetadataCore4270 : ExtensionRule
    {
        /// <summary>
        ...
        public override string Description
        {
            get
            {
                return "All bound functions with the same function name and binding parameter type within a namespace MUST specify the same return type.";
            }
        }
            ...
            foreach (var d in dic)
            {
                ...
                if (d.Value.Count >= 2)
                {
                    for (int i = 0; i < d.Value.Count - 1; i++)
                    {
                        ...
                        // Only get the first Parameter element from the function element.
                        xPath = "./*[local-name()='Parameter']";
                        var paramElem = funcElem1.XPathSelectElement(xPath, ODataNamespaceManager.Instance);
                        var bindingParamType1 = null != paramElem.Attribute("Type") ?
                            paramElem.GetAttributeValue("Type") : null;
                        paramElem = funcElem2.XPathSelectElement(xPath, ODataNamespaceManager.Instance);
                        var bindingParamType2 = null != paramElem.Attribute("Type") ?
                            paramElem.GetAttributeValue("Type") : null;
                        ...
                        if (null != parentElem1 && parentElem1 == parentElem2 &&
                            null != bindingParamType1 && bindingParamType1 == bindingParamType2 &&
