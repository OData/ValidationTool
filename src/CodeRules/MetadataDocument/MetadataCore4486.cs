...
namespace ODataValidator.Rule
{
    ...
    public class MetadataCore4486 : ExtensionRule
    {
        /// <summary>
        ...
        public override string Description
        {
            get
            {
                return "The last path segment MUST resolve either to a structural property in the context of the preceding path part, or to a term cast where the term MUST be of type Edm.ComplexType, Edm.PrimitiveType, a complex type, an enumeration type, a concrete primitive type, a type definition, or a collection of one of these types.";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            XmlNodeList NodeList = xmlDoc.SelectNodes("//*[local-name()='PropertyPath']");
            XmlNodeList AttributeNodeList = xmlDoc.SelectNodes("//*[@PropertyPath]");
            ...
            {
                string path;
                if (NvPropPath.Attributes["PropertyPath"] != null)
                {
                    path = NvPropPath.Attributes["PropertyPath"].Value;
                }
                else
                {
                ...
                if(parentOrTarget.LocalName.Equals("Annotations"))
                {
                    ...
                    if (MetadataHelper.Path(parentTargetPath, xmlDoc_MergedMetadata, context, ref parentOrTarget))
                    {
                        ...
                        {
                            if(resolveNode.LocalName.Equals("Property"))
                            {
                                passed = true; 
                            }
                            else if(resolveNode.LocalName.Equals("Term")) 
                            {
                                if (resolveNode.Attributes["Type"].Value.RemoveCollectionFlag().Equals("Edm.ComplexType") ||
                                    resolveNode.Attributes["Type"].Value.RemoveCollectionFlag().Equals("Edm.PrimitiveType"))
                                {
                                    passed = true;
                                }
                                else if(EdmTypeManager.IsEdmSimpleType(resolveNode.Attributes["Type"].Value.RemoveCollectionFlag()))
                                {
                                    passed = true;
                                }
                                else 
                                {
                                    ...
                                    termTypeEle = MetadataHelper.GetTypeDefinitionEleInScope(@"ComplexType' or 'EnumType' or 'TypeDefinition", type, context);
                                    if (termTypeEle != null)
                                    {
                                        passed = true;
                                    }
