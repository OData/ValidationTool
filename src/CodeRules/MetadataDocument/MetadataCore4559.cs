...
namespace ODataValidator.Rule
{
    ...
    public class MetadataCore4559 : ExtensionRule
    {
        /// <summary>
        ...
        public override string Description
        {
            get
            {
                return "A nominal type has a name that MUST be a SimpleIdentifier.";
            }
        }
            ...
            foreach (XmlNode schema in schemaList)
            {
                if (schema.HasChildNodes)
                {
                    foreach (XmlNode node in schema.ChildNodes)
                    {
                        ...
                        if (VerificationHelper.IsSimpleIdentifier(node.Attributes["Name"].Value))
                        {
                            passed = true;
                        }
                        else
