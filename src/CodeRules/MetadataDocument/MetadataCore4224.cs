...
namespace ODataValidator.Rule
{
    ...
    public class MetadataCore4224 : ExtensionRule
    {
        /// <summary>
        ...
        public override string Description
        {
            get
            {
                return "If the IsFlags attribute has a value of false, either all members MUST specify an integer value for the Value attribute, or all members MUST NOT specify a value for the Value attribute.";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            {
                if (enumType.Attributes["IsFlags"] != null && enumType.Attributes["IsFlags"].Value.Equals("false"))
                {
                    foreach (XmlNode member in enumType.ChildNodes)
                    {
                        ...
                        else
                        {
                            if (member.Attributes["Value"] != null)
                            {
                                try
                                {
                                    Convert.ToInt64(member.Attributes["Value"].Value);
                                    passed = true;
                                }
