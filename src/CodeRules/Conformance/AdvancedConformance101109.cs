...
namespace ODataValidator.Rule
{
    ...
    /// <summary>
    /// Class of extension rule for Advanced.Conformance.101109
    /// </summary>
    [Export(typeof(ExtensionRule))]
    public class AdvancedConformance101109 : ConformanceAdvancedExtensionRule
    {
        /// <summary>
        /// Gets rule name
        /// </summary>
        public override string Name
        {
            get
            {
                return "Advanced.Conformance.101109";
            }
        }
        ...
        public override string Description
        {
            get
            {
                return "7). A response to a batch request MUST contain a Content-Type header with value multipart/mixed. (section 11.7.4)";
            }
        }
...
", entityId);
                    ...
                    if (HttpStatusCode.OK == resp.StatusCode)
                    {
                        if (resp.ResponseHeaders.Contains("Content-Type: multipart/mixed"))
                        {
                            passed = true;
                        }
                        else
                        {
                            passed = false;
                            detail2.ErrorMessage = "The response headers does not contains 'Content-Type: multipart/mixed'.";
                        }
                    }
                    else
                    {
            ...
            {
                passed = false;
                detail1.ErrorMessage = string.Format("Created entity failed for above URI with entity data {0}. ", reqDataStr);
            }
