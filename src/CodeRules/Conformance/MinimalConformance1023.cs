...
namespace ODataValidator.Rule
{
    ...
    /// <summary>
    /// Class of extension rule for Minimal.Conformance.1023
    /// </summary>
    [Export(typeof(ExtensionRule))]
    public class MinimalConformance1023 : ConformanceMinimalExtensionRule
    {
        /// <summary>
        /// Gets rule name
        /// </summary>
        public override string Name
        {
            get
            {
                return "Minimal.Conformance.1023";
            }
        }
        ...
        public override string Description
        {
            get
            {
                return "23. MUST return a Location header with the edit link or read link of a created resource (section 11.4.2)";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            {
                string locationHeader = StringHelper.Filtration(resp.ResponseHeaders, "\nLocation: ");
                string locationHeaderVal = locationHeader.Remove(0, "Location: ".Length);
                if (Uri.IsWellFormedUriString(locationHeaderVal, UriKind.Absolute))
                {
                    ...
                    {
                        passed = false;
                        detail2.ErrorMessage = "Can not get the entity from above URI.";
                    }
                }
                else
                {
                    passed = false;
                    detail1.ErrorMessage = string.Format("The value of Location header {0} is not a absolute URI.", locationHeaderVal);
                }
