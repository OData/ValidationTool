...
namespace ODataValidator.Rule
{
    ...
    /// <summary>
    /// Class of extension rule for Advanced.Conformance.101117
    /// </summary>
    [Export(typeof(ExtensionRule))]
    public class AdvancedConformance101117 : ConformanceAdvancedExtensionRule
    {
        /// <summary>
        ...
        public override string Name
        {
            get
            {
                return "Advanced.Conformance.101117";
            }
        }
        ...
        public override string Description
        {
            get
            {
                return "10). When an error occurs processing a request and the odata.continue-on-error preference is not specified, processing of the batch is terminated and the error response is the last part of the multi-part response. (section 11.7.4)";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            {
                detail1.ErrorMessage = string.Format(Constants.ErrorMsg, "To verify this rule it expects an entity type with Int32/Int64/Int16/Guid/String key property and a normal property with string type, but there is not such entity type in metadata, so can not verify this rule.");
...
";
                        ...
                        {
                            string flag = "</m:error>";
                            int index = resp.ResponsePayload.IndexOf(flag);
                            string payload = resp.ResponsePayload.Remove(0, index + flag.Length);

                            if (0 == payload.IndexOf("\r\n--changesetresponse_"))
                            {
                                passed = true;
                            }
                            else
                            {
                                passed = false;
                                detail2.ErrorMessage = "Not found the batch operation terminated character following closely, when an error occurs in a changeset of a batch operation.";
                            }
                        }
                        else
                        {
