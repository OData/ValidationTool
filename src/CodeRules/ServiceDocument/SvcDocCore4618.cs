...
namespace ODataValidator.Rule
{
    ...
    /// <summary>
    /// Class of extension rule for SvcDoc.Core.4618
    /// </summary>
    [Export(typeof(ExtensionRule))]
    public class SvcDocCore4618 : ExtensionRule
    {
        /// <summary>
        ...
        public override string Description
        {
            get
            {
                return @"The metadata:name attribute of metadata:function-import element MUST contain the name of the function import.";
            }
        }
...
//</service>");
            ...
            XmlNodeList nameNodes = xmlDoc.SelectNodes(@"//app:workspace/metadata:function-import/@metadata:name", nsmgr);

            if (nameNodes.Count > 0)
            {
                string xpath = @"//*[local-name()='EntityContainer']/*[local-name()='FunctionImport']";
                ...
                {
                    if (functionImportNames.Contains(node.Value))
                    {
                        passed = true;
                    }
                    else 
