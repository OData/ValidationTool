...
namespace ODataValidator.Rule
{
    ...
    /// <summary>
    /// Class of extension rule for SvcDoc.Core.4630
    /// </summary>
    [Export(typeof(ExtensionRule))]
    public class SvcDocCore4630 : ExtensionRule
    {
        /// <summary>
        ...
        public override string Description
        {
            get
            {
                return @"OData represents function imports that are marked with IncludeInServiceDocument=""true"" as metadata:function-import elements contained within the app:workspace element.";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            string xpath = @"//*[local-name()='EntityContainer']/*[local-name()='FunctionImport' and @IncludeInServiceDocument='true']";
            List<string> functionImportNames = Helper.MetadataHelper.GetPropertyValues(context, xpath, "Name");

            if (functionImportNames.Count > 0)
            {
...
//                </service>");
                ...
                XmlNodeList nodes = xmlDoc.SelectNodes(@"//app:workspace/metadata:function-import", nsmgr);

                if (nodes.Count == functionImportNames.Count)
                {
                    passed = true;
                }
                else
