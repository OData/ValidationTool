...
namespace ODataValidator.Rule
{
    ...
    /// <summary>
    /// Class of extension rule for SvcDoc.Core.4628
    /// </summary>
    [Export(typeof(ExtensionRule))]
    public class SvcDocCore4628 : ExtensionRule
    {
        /// <summary>
        ...
        public override string Description
        {
            get
            {
                return @"The atom:title element SHOULD contain a human-readable description of the singleton which MAY be the name of the singleton.";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            XmlNodeList nameNodes = xmlDoc.SelectNodes(@"//app:workspace/metadata:singleton/atom:title", nsmgr);
            ...
            if (nameNodes.Count > 0)
            {
                ...
                {
                    if (singletonNames.Contains(node.InnerText))
                    {
                        passed = true;
                    }
                    else
