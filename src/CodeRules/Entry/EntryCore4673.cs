...
namespace ODataValidator.Rule
{
    ...
    /// <summary>
    /// Class of entension rule for Entry.Core.4673
    /// </summary>
    [Export(typeof(ExtensionRule))]
    public class EntryCore4673 : ExtensionRule
    {
        /// <summary>
        ...
        public override string Description
        {
            get
            {
                return @"The title attribute of atom:link element for stream properties MAY be present.";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            foreach (var xe in StreamTypeResultList)
            {
                ...
                if (xereadlink != null)
                {
                    string title = xereadlink.GetAttributeValue(@"title");
                    if (title == null)
                    {
                        passed = false;
                        break;
                ...
                if (xeeditlink != null)
                {
                    string title = xeeditlink.GetAttributeValue(@"title");
                    if (title == null)
                    {
                        passed = false;
                        break;
