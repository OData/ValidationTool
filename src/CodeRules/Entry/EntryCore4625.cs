...
namespace ODataValidator.Rule
{
    ...
    /// <summary>
    /// Class of extension rule for Entry.Core.4625
    /// </summary>
    [Export(typeof(ExtensionRule))]
    public class EntryCore4625 : ExtensionRule
    {
        /// <summary>
        ...
        public override string Description
        {
            get
            {
                return @"An empty metadata:element element for Complex Property Collection with the metadata:null='true' attribute value represents a null value within the collection.";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            foreach (XmlNode xmlNode in xmlNodeList)
            {
                if (namesWithCollectionType.Contains(xmlNode.LocalName) && xmlNode.NamespaceURI.Equals(DataNs) && xmlNode.ChildNodes.Count > 0)
                {
                    ...
                    {
                        if (xElement.NamespaceURI.Equals(MeataNS) && xElement.LocalName.Equals("element") && xElement.Attributes["null", MeataNS] != null)
                        {
