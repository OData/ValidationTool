...
namespace ODataValidator.Rule
{
    ...
    /// <summary>
    /// Class of extension rule for Entry.Core.4099
    /// </summary>
    [Export(typeof(ExtensionRule))]
    public class EntryCore4099 : ExtensionRule
    {
        /// <summary>
        ...
        public override string Description
        {
            get
            {
                return "The name of association link MUST be the name of the property, followed by @odata.associationLink.";
            }
        }
            ...
            if (entry != null && entry.Type == JTokenType.Object)
            {
                ...
                foreach (JProperty jProp in jProps)
                {
                    if (jProp.Name.Contains(Constants.OdataAssociationLinkPropertyNameSuffix))
                    {
                        ...
                        {
                            if (jProp.Name == propNode.Attribute("Name").Value + Constants.OdataAssociationLinkPropertyNameSuffix ||
                                jProp.Name == propNode.Attribute("Name").Value + Constants.OdataAssociationLinkPropertyNameSuffix + @"Url")
                            {
                                passed = true;
                            }
                            else
