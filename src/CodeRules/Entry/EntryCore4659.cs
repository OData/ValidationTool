...
namespace ODataValidator.Rule
{
    ...
    /// <summary>
    /// Class of entension rule for Entry.Core.4659
    /// </summary>
    [Export(typeof(ExtensionRule))]
    public class EntryCore4659 : ExtensionRule
    {
        /// <summary>
        ...
        public override string Description
        {
            get
            {
                return @"An empty collection of entities MUST be represented as an empty atom:feed in an expanded navigation property.";
            }
        }
                    ...
                    foreach (XmlNode le in linkElements)
                    {
                        ...
                        if (collectionTypeNavigPropNames.Contains(le.Attributes["title"].Value) && expandQueryVals.Contains(le.Attributes["title"].Value))
                        {
                            if (le.FirstChild == le.LastChild && metadataNp + @":inline" == le.FirstChild.Name)
                            {
                                ...
                                XmlNode feedNode = inlineNode.SelectSingleNode(@"./atom:feed", ODataNamespaceManager.Instance);

                                if (!feedNode.HasChildNodes)
                                {
                                    passed = true;
                                }
                            }
