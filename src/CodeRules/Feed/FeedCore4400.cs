...
namespace ODataValidator.Rule
{
    ...
    /// <summary>
    /// Class of extension rule for Feed.Core.4400
    /// </summary>
    [Export(typeof(ExtensionRule))]
    public class FeedCore4400 : ExtensionRule
    {
        /// <summary>
        ...
        public override string Description
        {
            get
            {
                return "The odata.context annotation MUST also be included for entities whose entity set cannot be determined from the context URL of the collection in V4.";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            if (odatametadata.Name.Equals(Constants.OdataV4JsonIdentity))
            {
                // Get entities name.
                ...
                foreach (JObject entry in entries)
                {
                    if (entry != null && entry.Type == JTokenType.Object)
                    {
                        ...
                        {
                            if (jProp.Name.Equals(Constants.OdataV4JsonIdentity))
                            {
                                ...
                                if (splitPart.EndsWith(Constants.V4JsonEntityIdentity))
                                {
