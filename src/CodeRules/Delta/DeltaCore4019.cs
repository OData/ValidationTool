...
namespace ODataValidator.Rule
{
    ...
    /// <summary>
    /// Class of extension rule for Delta.Core.4019
    /// </summary>
    [Export(typeof(ExtensionRule))]
    public class DeltaCore4019 : ExtensionRule
    {
        /// <summary>
        ...
        public override string Description
        {
            get
            {
                return @"The deleted-entity object MUST include id, the id of the deleted entity, which may be absolute or relative.";
            }
        }
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            {               
                foreach (JObject entity in (JArray)jo[Constants.Value])
                {
                    ...
                    {
                        if (entity[Constants.ID] != null && Uri.IsWellFormedUriString(entity[Constants.ID].Value<string>().StripOffDoubleQuotes(), UriKind.RelativeOrAbsolute))
                        {
                            passed = true;
                        }
                        else
